            ...
            <div class="album py-5 bg-light">
                <div class="container">
                    <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 g-3">    
                        ...
                        @foreach ($multimusica as $musica)      
                            <div class="col">
                                <div class="card shadow-sm">
                                    <img src={{ $musica->Poster }}>
                                        <div class="card-body">
                                        <p class="card-text">{{ $musica->Title }}</p>
                                        <p class="card-text text-truncate">{{ $musica->Description }}</p>
                                            <div class="d-flex justify-content-between align-items-center">
                                                    <div class="btn-group">
                                                    <button type="button" class="btn btn-sm btn-outline-secondary" id={{ $musica->Id }} >Подробнее</button>
                                                    </div>
                                                <small class="text-muted">{{ $musica->Time }}</small>
                                            </div>
                                        </div>
                                </div>
                            </div>    
                            <script>                    //выводим данные Laravel (из БД) в JS
                                arr = {               
                                        'id': `{{ $musica->Id  }}`,
                                        'Premiere': `{{ $musica->Premiere  }}`,
                                        'Starring': `{{ $musica->Starring  }}`,
                                        'Time': `{{ $musica->Time  }}`,
                                        'Country': `{{ $musica->Country  }}`,
                                        'Title': `{{ $musica->Title  }}`,
                                        'Poster': `{{ $musica->Poster  }}`,
                                        'Description': `{{ $musica->Description  }}` 
                                        };        
                                        poster[ {{$musica->Id}} ] = arr     //добавляем обьект в массив
                                console.log(poster);
                            </script>
                        @endforeach
                    </div>        
                ...
                if (target.tagName == 'BUTTON') {                       //если это кнопка 
                    ...
                    completeEventInformation.innerHTML = `<div><img style="width: 600px; margin: 10px" src="${poster[elemId].Poster}" alt="image"></div>
                                                        <div style="display:flex; flex-direction:column;">
                                                            ...
                                                            <div><a class="btn btn-sm btn-outline-secondary" href='http://127.0.0.1:8000/FavoritesMusic/${poster[elemId].id}'>Add to favourites</a></div>`; 
