                        ...
                        @foreach ($multifilm as $film)      
                                    ...
                                    <img src={{ $film->Poster }}>
                                        ...
                                        <p class="card-text text-truncate">{{ $film->Description }}</p>
                                            <div class="d-flex justify-content-between align-items-center">
                                                    <div class="btn-group">
                                                    <button type="button" class="btn btn-sm btn-outline-secondary" id={{ $film->Id }} >Подробнее</button>
                                                    </div>
                                                <small class="text-muted">{{ $film->Time }}</small>
                                            </div>
                                        </div>
                                </div>
                            </div>    
                            <script>                    //выводим данные Laravel (из БД) в JS
                                arr = {               
                                        ...
                                        poster[ {{$film->Id}} ] = arr;     //добавляем обьект в массив
                                console.log(poster);
                            </script>
                        @endforeach
                    </div>        
                ...
                if (target.tagName == 'BUTTON') {                       //если это кнопка 
                    ...
                    completeEventInformation.innerHTML = `<div><img style="width: 600px; margin: 10px" src="${poster[elemId].Poster}" alt="image"></div>
                                                        <div style="display:flex; flex-direction:column;">
                                                            ...
                                                            <div><a class="btn btn-sm btn-outline-secondary" href='http://127.0.0.1:8000/FavoritesFilm/${poster[elemId].id}'>Add to favourites</a></div>`; 
